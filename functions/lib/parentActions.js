...
exports.linkChild = functions.https.onCall(async (data, context) => {
    ...
    if (context.auth.token.email_verified !== true) {
        throw new functions.https.HttpsError('failed-precondition', 'Please verify your email address first — check your inbox for the verification link, then try again.');
    }
    const { playerId } = data;
    if (!playerId) {
        throw new functions.https.HttpsError('invalid-argument', 'playerId is required');
